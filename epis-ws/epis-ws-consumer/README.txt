...
1. jar 파일 실행
	...
	예) Configuration 동기화 작업을 실행할 경우
		=> java -Dconsumer.root.dir=C:\Sites\Works\episWorks\epis-ws\epis-ws-consumer -Djob.name=job1 -jar epis-ws-consumer.jar Config
	
	※문자열 표시는 '[',']'으로 감싸서 표시하며, 실제 적용시에는 '[',']'기호는 들어가지 않음
	※값에대한 설명은 '{','}'으로 감싸서 표시함
...
3. DataSource 처리
	...
	- system property (-Djob.name)을 이용하여 job name에 따라 db접속처리 함
